...
def get_by_paypal_id(paypal_order_id: str):
    """webhook 用：這時還不知道是哪個 caller，由這筆資料告訴我們。"""
    return db.query("SELECT * FROM orders WHERE paypal_order_id = %s",
                    (paypal_order_id,), fetch="one")


def create(caller_id: str, reference_id: str, amount, currency: str, status: str):
    return db.query(
        "INSERT INTO orders (caller_id, reference_id, amount, currency, status)"
        " VALUES (%s, %s, %s, %s, %s) RETURNING *",
        (caller_id, reference_id, amount, currency, status), fetch="one")


def attach_paypal_id(order_id: str, paypal_order_id: str, status: str):
    return db.query(
        "UPDATE orders SET paypal_order_id = %s, status = %s, updated_at = now()"
        " WHERE id = %s RETURNING *",
        (paypal_order_id, status, order_id), fetch="one")


def set_status(order_id: str, status: str, captured: bool = False):
    return db.query(
        "UPDATE orders SET status = %s, updated_at = now(),"
        " captured_at = CASE WHEN %s THEN now() ELSE captured_at END"
        " WHERE id = %s RETURNING *",
        (status, captured, order_id), fetch="one")
